...
### [x] 精确编辑 + rg 搜索 + 过期检查
- **问题**：SwayBot 已有 `read_file`/`write_file`，但缺少安全、可复现的代码编辑和内容搜索能力。
- **方案**：参考 OpenCode 核心层做法，新增 `edit_file`（精确字符串替换，检查旧字符串唯一性，避免基于过期内容修改）、`grep`（优先 `rg --json`，回退 stdlib 递归搜索）。
- **文件**：`swaybot/tools/fs.py`、`swaybot/tools/__init__.py`、`tests/test_fs.py`、`tests/test_tools.py`、`tests/test_agent.py`
- **验收**：`edit_file` 单次/全量替换、过期/多匹配拒绝；`grep` 在有无 `rg` 环境下都能返回 `file:line: text`；默认工具注册包含新工具。255 个测试全部通过。
- **状态**：已完成（2026-07-17）。